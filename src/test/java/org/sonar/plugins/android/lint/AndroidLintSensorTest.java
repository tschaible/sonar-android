...
public class AndroidLintSensorTest {
  ...
  public void shouldOnlyRunOnJavaModules() throws Exception {
    when(rulesProfile.getActiveRulesByRepository(AndroidLintRulesDefinition.REPOSITORY_KEY)).thenReturn(Arrays.asList(new ActiveRule()));
  ...
  public void shouldNotRunIfNoAndroidRule() throws Exception {
    ...
    when(rulesProfile.getActiveRulesByRepository(AndroidLintRulesDefinition.REPOSITORY_KEY)).thenReturn(Collections.emptyList());
    assertThat(sensor.shouldExecuteOnProject(project)).isFalse();
